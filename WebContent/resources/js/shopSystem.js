		...
		$(function(){
			$.ajax({
				...
				success : function(data){
					...
					str += `
					<tr>
					<td onclick="getemAll('${select}','${seller}')">전체</td>
					</tr>`;
					for(var i=0; i<jsondata.category.length; i++){
					
						str += `
						<tr>
							<td onclick="getMidCategory('${jsondata.category[i]}','${seller}')"'>${jsondata.category[i]}</td>
						</tr>`;
					}
				...
				success:function(data){
					...
					for(var i=0;i<jsondata.item.length;i++){
						...
						str += `<div class="itemInfo">
									...
									<div class="itemPrice">${list1}원</div>
								  </div>`;						
					}
					...
					var pagestr="";
						...
						for(var i=startPage;i<=endPage;i++){
								pagestr +=`<span class="page" style='cursor:pointer;' onclick='getAllitemList("${seller}", "`+i+`")'>`+i+`</span>`
							}
						pagestr += `</div>`;
						if(endPage < totalPage){
							nowPage = endPage + 1;
		...
		function getMidCategory(select,seller,nowpage){
			...
			$(function(){
				...
				var pagestr="";
						...
						pagestr += `<div class="pagination">`;
						for(var i=startPage;i<=endPage;i++){
								pagestr +=`<span class="page" style='cursor:pointer;' onclick='getMidCategory("${seller}", "`+i+`")'>`+i+`</span>`
							}
						pagestr += `</div>`;
						if(endPage < totalPage){
							nowPage = endPage + 1;
					...
					success:function(data){
						var jsondata = JSON.parse(data);
							...
							str += `<div class="itemInfo">
											  ...
											  <div class="itemPrice">${list2}원</div>
										  </div>`;
						}
			...
			$(function(){
				$.ajax({
					...
					success:function(data){
						...
						str += `
						<tr>
						<td onclick="getMidCategory('${jsondata.AllCategory[i]}','${seller}')">${jsondata.AllCategory[i]}
						</td>
						</tr>`;
					}
					$(".midcatBorder_Left").html(str);
